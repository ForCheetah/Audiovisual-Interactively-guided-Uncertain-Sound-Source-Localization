...
class AudioExtract(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(AudioExtract, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
def audio_extract(pretrained=True, **kwargs):
    model = AudioExtract(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
...
if __name__ == "__main__":
    model = audio_extract()
    print("Model loaded.")
    image = Variable(torch.rand(2, 1, 256, 256))
    print("Image loaded.")
