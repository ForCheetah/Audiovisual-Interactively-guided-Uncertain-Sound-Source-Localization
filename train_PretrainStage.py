...
from models.Localization_Pretrain_Model import LocalizationModel
import torch
...
def create_optimizer(nets, opt):
    (net_image_extract, net_audio_extract, net_AVattention, net_Audio_Separation, net_Align) = nets
    param_groups = [
...
net_Align = builder.build_Align(
        mode='train',
        weights="")
nets = (net_image_extract, net_audio_extract, net_AVattention, net_Audio_Separation, net_Align)
...
crit = {'unsuperviseloss': unsuperviseloss, 'loss_align': loss_align}
...
#         SaveObject(output, data['vid'])
        ...
        if (total_batches % opt.display_freq == 0):
            ...
            print('unsupervise loss: %.4f, align loss: %.4f' \
                  % (avg_unsupervise_loss, avg_align_loss))
            batch_unsupervise_loss = []
            batch_align_loss = []
            batch_ce_loss = []
            if opt.tensorboard:
