...
class TrainOptions(BaseOptions):
	def initialize(self):
		...
		self.parser.add_argument('--save_latest_freq', type=int, default=180, help='frequency of saving the latest results')
		self.parser.add_argument('--continue_train', action='store_true', help='continue training: load the latest model')
		self.parser.add_argument('--epoch_count', type=int, default=0, help='the starting epoch count')
		self.parser.add_argument('--epoch', type=int, default=1, help='the total epoch count')
		...
		self.parser.add_argument('--validation_freq', type=int, default=600, help='frequency of testing on validation set')
		self.parser.add_argument('--validation_batches', type=int, default=20, help='number of batches to test for validation')
		self.parser.add_argument('--validation_visualization', type=bool, default=True, help='whether save validation predictions')
		...
		self.parser.add_argument('--weighted_loss', type=bool, default=True, help="weighted loss")
		...
		self.parser.add_argument('--with_additional_scene_image',type=bool, default=False, help="whether to append an extra scene image")

		#optimizer arguments
		self.parser.add_argument('--lr_visual', type=float, default=0.00001, help='learning rate for visual stream')
		self.parser.add_argument('--lr_unet', type=float, default=0.0001, help='learning rate for unet')
		self.parser.add_argument('--lr_classifier', type=float, default=0.0001, help='learning rate for audio classifier')
		self.parser.add_argument('--lr_steps', nargs='+', type=int, default=[10000, 25000], help='steps to drop LR in training samples')
		...
		self.parser.add_argument('--lr_localization', type=float, default=0.00001, help='learning rate for unet')

		# option in stage two
		self.parser.add_argument('--simLoss_weight', default=1, type=float, help='weight for simLoss loss')
		self.parser.add_argument('--constraintloss_weight', default=1, type=float, help='weight for constraint loss')
		self.parser.add_argument('--ceLossImage_weight', default=1, type=float, help='weight for celoss image loss')
		self.parser.add_argument('--celossAudio_weight', default=1, type=float, help='weight for celoss audio loss')
